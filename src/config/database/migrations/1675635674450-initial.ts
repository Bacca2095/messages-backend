...
export class migrations1675573172429 implements MigrationInterface {
  name = 'migrations1675573172429';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`campaign_contact\` (\`id\` int NOT NULL AUTO_INCREMENT, \`contactId\` int NULL, \`campaignId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`custom_field\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`clientId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`emailVerified\` tinyint NOT NULL DEFAULT 0, \`password\` varchar(255) NOT NULL, \`status\` tinyint NOT NULL DEFAULT 1, \`phoneNumber\` varchar(255) NOT NULL, \`regionCode\` enum ('CO') NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`clientId\` int NULL, UNIQUE INDEX \`IDX_0f6c11be238ad75c8bd2bd0245\` (\`email\`, \`clientId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`campaign_contact\` ADD CONSTRAINT \`FK_cb49cd9fc28410f741961d8bd43\` FOREIGN KEY (\`contactId\`) REFERENCES \`contact\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`campaign_contact\` ADD CONSTRAINT \`FK_8c3cc7d5cc88c8561a3c6fa0cd3\` FOREIGN KEY (\`campaignId\`) REFERENCES \`campaign\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`campaign_contact\` DROP FOREIGN KEY \`FK_8c3cc7d5cc88c8561a3c6fa0cd3\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`campaign_contact\` DROP FOREIGN KEY \`FK_cb49cd9fc28410f741961d8bd43\``,
    );
    await queryRunner.query(
