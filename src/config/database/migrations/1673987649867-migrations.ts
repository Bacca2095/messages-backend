...
export class migrations1673987649867 implements MigrationInterface {
  name = 'migrations1673987649867';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `user` CHANGE `emailVerified` `emailVerified` tinyint NOT NULL DEFAULT 0',
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `user` CHANGE `emailVerified` `emailVerified` tinyint NOT NULL',
    );
    await queryRunner.query(
      'CREATE UNIQUE INDEX `IDX_0f6c11be238ad75c8bd2bd0245` ON `user` (`email`, `clientId`)',
    );
