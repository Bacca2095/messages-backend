...
export class initial1675821636226 implements MigrationInterface {
    name = 'initial1675821636226'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`custom_field\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`inputType\` enum ('TEXT', 'TEXTAREA', 'SELECT', 'DATE', 'TIME') NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`clientId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`emailVerified\` tinyint NOT NULL DEFAULT 0, \`password\` varchar(255) NOT NULL, \`status\` tinyint NOT NULL DEFAULT 1, \`phoneNumber\` varchar(255) NOT NULL, \`provider\` enum ('google') NULL, \`socialId\` varchar(255) NULL, \`regionCode\` enum ('CO') NOT NULL, \`resetPasswordToken\` varchar(255) NULL, \`resetPasswordExpires\` timestamp NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`clientId\` int NULL, UNIQUE INDEX \`IDX_0f6c11be238ad75c8bd2bd0245\` (\`email\`, \`clientId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`payment\` (\`id\` int NOT NULL AUTO_INCREMENT, \`date\` timestamp NOT NULL, \`amount\` decimal NOT NULL, \`clientSubscriptionId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`client-subscription\` (\`id\` int NOT NULL AUTO_INCREMENT, \`startDate\` timestamp NOT NULL, \`endDate\` timestamp NOT NULL, \`smsRemaining\` int NOT NULL, \`status\` varchar(255) NOT NULL, \`clientId\` int NULL, \`subscriptionId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`subscription\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`price\` decimal NOT NULL, \`smsPrice\` int NOT NULL, \`description\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`constrains\` (\`id\` int NOT NULL AUTO_INCREMENT, \`field\` varchar(255) NOT NULL, \`value\` varchar(255) NOT NULL, \`condition\` enum ('between', 'equal', 'less', 'more', 'contain') NOT NULL, \`campaignId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`campaign\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`message\` varchar(255) NOT NULL, \`startDate\` datetime NOT NULL, \`endDate\` datetime NOT NULL, \`startTime\` time NOT NULL, \`endTime\` time NOT NULL, \`days\` text NOT NULL, \`status\` enum ('pending', 'finalized', 'paused', 'sending') NOT NULL DEFAULT 'pending', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`clientId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`payment\` ADD CONSTRAINT \`FK_192fc09aa38cc8c0ec778d4b90d\` FOREIGN KEY (\`clientSubscriptionId\`) REFERENCES \`client-subscription\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`client-subscription\` ADD CONSTRAINT \`FK_e70fa3a0132a87e05b663d47087\` FOREIGN KEY (\`clientId\`) REFERENCES \`client\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`client-subscription\` ADD CONSTRAINT \`FK_1a16a6680f64bd94f446a58ab29\` FOREIGN KEY (\`subscriptionId\`) REFERENCES \`subscription\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`constrains\` ADD CONSTRAINT \`FK_f71e2fc6351e037701da7be9cdb\` FOREIGN KEY (\`campaignId\`) REFERENCES \`campaign\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`constrains\` DROP FOREIGN KEY \`FK_f71e2fc6351e037701da7be9cdb\``);
        await queryRunner.query(`ALTER TABLE \`client-subscription\` DROP FOREIGN KEY \`FK_1a16a6680f64bd94f446a58ab29\``);
        await queryRunner.query(`ALTER TABLE \`client-subscription\` DROP FOREIGN KEY \`FK_e70fa3a0132a87e05b663d47087\``);
        await queryRunner.query(`ALTER TABLE \`payment\` DROP FOREIGN KEY \`FK_192fc09aa38cc8c0ec778d4b90d\``);
