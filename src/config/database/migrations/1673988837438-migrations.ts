...
export class migrations1673988837438 implements MigrationInterface {
  name = 'migrations1673988837438';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `client` CHANGE `emailVerified` `emailVerified` bit NOT NULL DEFAULT 0',
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      "ALTER TABLE `client` CHANGE `emailVerified` `emailVerified` bit NOT NULL DEFAULT 'b'0''",
    );
    await queryRunner.query(
    ...
    await queryRunner.query('ALTER TABLE `user` DROP COLUMN `status`');
    await queryRunner.query(
      "ALTER TABLE `user` ADD `status` bit NOT NULL DEFAULT 'b'1''",
    );
    await queryRunner.query(
      'CREATE UNIQUE INDEX `IDX_0f6c11be238ad75c8bd2bd0245` ON `user` (`email`, `clientId`)',
    );
